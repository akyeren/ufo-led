...
void setup() {
  ...
  FastLED.addLeds<LED_TYPE, LED_PIN_RING, COLOR_ORDER>(ledsRing, NUM_LEDS_RING); //.setCorrection( TypicalLEDStrip );
...
void lauflicht(int iPoti, int *i_Speed, int i, int j)
{
  ...
  *i_Speed  = map(iPoti, 0, 600, 0, 150);
  ...
  for (int k = 0; k < ANZ_SEGMENTE; k++) {
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 0) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( L_MAX - c, iBrightness));
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 1) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( L_MAX - b, iBrightness));
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 2) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( L_MAX - a, iBrightness));
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 3) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( c, iBrightness));
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 4) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( b, iBrightness));
    leds[((NUM_LEDS + i + (k * NUM_LEDS / ANZ_SEGMENTE)) + 5) % NUM_LEDS] = CHSV(iColorBigRing, iHue, min( a, iBrightness));
  }
  ...
  for (int k = 0; k < ANZ_SEGMENTE; k++) {
    leds[((NUM_LEDS + i) + (k * NUM_LEDS / ANZ_SEGMENTE)) % NUM_LEDS] = CRGB::Black;
  }
